...
pub enum TableFactor {
    ...
    /// Snowflake's `RESAMPLE` operation on a table.
    Resample(Box<Resample>),
...
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct Resample {
    /// The input table to resample.
    pub table: Box<TableFactor>,
    /// The column defining the sampling axis.
    pub using: Ident,
    /// The interval or numeric increment between generated rows.
    pub increment: Expr,
    /// Columns that define independent partitions.
    pub partition_by: Vec<Ident>,
    /// Metadata columns emitted by the operation.
    pub metadata_columns: Vec<ResampleMetadataColumn>,
    /// Optional alias for the resulting table.
    pub alias: Option<TableAlias>,
}

/// A metadata column requested by a `RESAMPLE` table factor.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct ResampleMetadataColumn {
    /// The metadata value to emit.
    pub kind: ResampleMetadataColumnKind,
    /// Optional output column alias.
    pub alias: Option<Ident>,
}

/// The metadata values available from `RESAMPLE`.
#[derive(Debug, Clone, Copy, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub enum ResampleMetadataColumnKind {
    /// Whether the row was generated by resampling.
    IsGenerated,
    /// The beginning of the row's sampling bucket.
    BucketStart,
}

impl fmt::Display for ResampleMetadataColumnKind {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::IsGenerated => write!(f, "IS_GENERATED()"),
            Self::BucketStart => write!(f, "BUCKET_START()"),
        }
    }
}

impl fmt::Display for ResampleMetadataColumn {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.kind)?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            TableFactor::Resample(resample) => {
                let Resample { table, using, increment, partition_by, metadata_columns, alias } =
                    resample.as_ref();
                write!(f, "{table} RESAMPLE(USING {using} INCREMENT BY {increment}")?;
                if !partition_by.is_empty() {
                    write!(f, " PARTITION BY {}", display_comma_separated(partition_by))?;
                }
                if !metadata_columns.is_empty() {
                    write!(f, " METADATA_COLUMNS {}", display_comma_separated(metadata_columns))?;
                }
                write!(f, ")")?;
                if let Some(alias) = alias {
                    write!(f, " {alias}")?;
