...
impl fmt::Display for EscapeQuotedString<'_> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // The quoted-string AST nodes hold the *decoded* value, so escaping is
        // the exact inverse of parsing: double every occurrence of the quote
        // character. Doubling unconditionally keeps adjacent quotes — such as an
        // embedded empty-string literal `''` — correct. A peek-ahead
        // "already escaped" heuristic would mistake such a pair for a
        // pre-escaped quote and leave it under-escaped.
        let quote = self.quote;
        let mut start_idx = 0;
        for (idx, ch) in self.string.char_indices() {
            if ch == quote {
                // Including idx in the range emits the quote at idx, then
                // start_idx is reset to idx so the next write emits it again.
