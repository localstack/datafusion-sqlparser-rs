...
impl<'a> Parser<'a> {
    ...
    pub fn parse_drop(&mut self) -> Result<Statement, ParserError> {
        ...
        let object_type = if self.parse_keyword(Keyword::TABLE) {
            ...
            return self.expected(
                "CONNECTOR, DATABASE, EXTENSION, FUNCTION, INDEX, OPERATOR, POLICY, PROCEDURE, ROLE, SCHEMA, SECRET, SEQUENCE, STAGE, TABLE, TRIGGER, TYPE, VIEW, MATERIALIZED VIEW, USER or WAREHOUSE after DROP",
                self.peek_token(),
            );
        };
    ...
    pub fn parse_alter(&mut self) -> Result<Statement, ParserError> {
        ...
        match object_type {
            ...
            // unreachable because expect_one_of_keywords used above
            unexpected_keyword => Err(ParserError::ParserError(
                format!("Internal parser error: expected any of {{VIEW, TYPE, TABLE, INDEX, ROLE, POLICY, CONNECTOR, ICEBERG, SCHEMA, USER, OPERATOR, WAREHOUSE}}, got {unexpected_keyword:?}"),
            )),
        }
    }
